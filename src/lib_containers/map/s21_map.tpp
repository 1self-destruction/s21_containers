...
map<key_type, mapped_type>::map(const map& m) {
  size_ = m.size_;
  root = new node_pair<key_type, mapped_type>(m.root->pair, nullptr, nullptr);
  ...
  while (current != nullptr) {
    if (value.first < current->pair.first) {
      if (!current->left) {
        ...
        current->left =
            new node_pair<key_type, mapped_type>(value, nullptr, nullptr);
        size_++;
...
map<key_type, mapped_type>::insert(const key_type& key,
  ...
  while (current != nullptr) {
    if (current->pair.first > key) {
      if (!current->left) {
        ...
        current->left = new node_pair<key_type, mapped_type>(
            std::make_pair(key, obj), nullptr, nullptr);
        size_++;
        iterator iter(current->left, this);
        return std::make_pair(iter, true);
      }
      ...
      if (!current->right) {
        ...
        current->right = new node_pair<key_type, mapped_type>(
            std::make_pair(key, obj), nullptr, nullptr);
        size_++;
        iterator iter(current->right, this);
        return std::make_pair(iter, true);
  ...
  root = new node_pair<key_type, mapped_type>(std::make_pair(key, obj), nullptr,
                                              nullptr);
  size_++;
  iterator iter(root, this);
  return std::make_pair(iter, true);
...
template <typename key_type, typename mapped_type>
void map<key_type, mapped_type>::erase(
    typename map<key_type, mapped_type>::iterator pos) {
...
map<key_type, mapped_type>::const_begin() {
...
template <typename key_type, typename mapped_type>
typename map<key_type, mapped_type>::const_iterator
map<key_type, mapped_type>::const_end() {
...
template <typename key_type, typename mapped_type>
void map<key_type, mapped_type>::copy_map(
    node_pair<key_type, mapped_type>* current_from,
    node_pair<key_type, mapped_type>*& current_to) {
  if (current_from->left != nullptr) {
    current_to->left = new node_pair<key_type, mapped_type>(
        current_from->left->pair, nullptr, nullptr);
    copy_map(current_from->left, current_to->left);
  }
  if (current_from->right != nullptr) {
    current_to->right = new node_pair<key_type, mapped_type>(
        current_from->right->pair, nullptr, nullptr);
    copy_map(current_from->right, current_to->right);
  }
}

template <typename key_type, typename mapped_type>
void map<key_type, mapped_type>::clear_recursive(
...
template <typename key_type, typename mapped_type>
void map<key_type, mapped_type>::from_tree_to_array(
    std::vector<node_pair<key_type, mapped_type>>& vec,
    node_pair<key_type, mapped_type>* current,
...
template <typename key_type, typename mapped_type>
void map<key_type, mapped_type>::balanced_tree_recursive(
    std::vector<node_pair<key_type, mapped_type>> vec) {
  if (vec.size() == 1) {
    insert(vec[0].pair);
  } else if (vec.size() == 2) {
